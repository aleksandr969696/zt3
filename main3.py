...
def boundary(x, on_boundary):
    if on_boundary:
        if near(x[0],0,tol) or near([x[0],math.pi/2,tol]):
            return True
        else:
            return False
    else:
...
u_D = sympy.sin(x)*sympy.cos(y) + sympy.sin(y)*sympy.cos(x)
# u_e = x**2+y**2
# u_e = x**2-y**2
# u_e = 0
u_e = Expression('x[0]==0 ? sin(x[1]) : cos(x[1])', degree=2)

# circle = Circle(Point(0, 0), r)
circle = Rectangle(Point(0,0),Point(math.pi/2,math.pi*3/2))
mesh = generate_mesh(circle, 30)
V = FunctionSpace(mesh, 'P', 2)
u_D = Expression(ccode(u_D), degree=2)
# u_D = 0
...
f = -2*sympy.sin(x)*sympy.cos(y)-2*sympy.sin(y)*sympy.cos(x)
# f = 0
...
#     degree=2)
g = Expression('x[1]==0 ? cos(x[0]) : -sin(x[0])', degree=2)
